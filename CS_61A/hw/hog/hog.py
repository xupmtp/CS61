...
def free_bacon(score):
    ...
    若擲骰子數量0,則根據對手當回分數決定 0: 3+3分 其他(n): PI小數後第n位值
    """
...
def swine_align(player_score, opponent_score):
    ...
    if player_score == 0 or opponent_score == 0:
        return False
    if player_score == opponent_score and player_score >= 10:
        return True
    gcd = max(i for i in range(1, max(opponent_score, player_score) // 2 + 1) if player_score % i == 0 and opponent_score % i == 0)
